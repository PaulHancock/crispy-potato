...
from math import sin,cos,tan,pi, acos,asin,atan, floor
...
def dec_to_dms(x):
	...
	m=int(floor((x-d)*60))
	s=float(( (x-d)*60-m)*60)
	return '{0:+02d}:{1:02d}:{02:5.4f}'.format(sign*d,m,s)

def dec_to_hms(x):
	h=int(floor(x/15.0))
	m=int(floor( (x/15.0-h)*60) )
	#m=int((x-d)*60)
	s=float( (x/15-h)*60-m)*60
	return	'{0:02d}:{1:02d}:{02:5.2f}'.format(h,m,s)
