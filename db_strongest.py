...
if __name__=="__main__":
    ...
    #master sources with no match in op.sf_name
    query.append("""SELECT m.src_id,m.ra,m.dec,m.flux,m.bmaj,m.bmin,m.pa,m.field
     FROM
      (SELECT * FROM master WHERE field=0 ) m
      LEFT OUTER JOIN
      (SELECT * FROM links WHERE sf='{0}' AND direction=0 AND field=0) l
      ON l.master_id = m.src_id 
     WHERE l.catalog_id IS NULL ORDER BY -m.flux LIMIT 100""".format(op.sf_name))
    #catalog sources with no match in master
    query.append("""SELECT c.src_id,c.ra,c.dec,c.pflux,c.bmaj,c.bmin,c.pa,c.field
     FROM
       (SELECT * FROM catalog WHERE sf='{0}' AND field=0) c
       LEFT OUTER JOIN
       (SELECT * FROM links WHERE direction=1 AND field=0) l
       ON l.catalog_id = c.src_id
       WHERE l.master_id IS NULL ORDER BY -c.pflux LIMIT 100""".format(op.sf_name))
    for q in range(len(query)):
        data=list(c.execute(query[q]))
        if q==0:
            prefix='M-{0}'.format(op.sf_name.upper())
            continue
        elif q==1:
        ...
        for row in data:
            src_id,ra,dec,flux,bmaj,bmin,pa,field = row
